...
public partial class patch_GUIOptions : GUIOptions {
    ...
    private void Init() {
        orig_Init();
        if (!removedExcess && tfList != null) {
            foreach (Transform tfEntry in tfList) {
                GUIModRow tfModRow = tfEntry.GetComponent<GUIModRow>();
                if (tfModRow != null && tfModRow.txtName.text.ToLower() == "core") {
                    GameObject.Destroy(tfEntry.gameObject);
                    removedExcess = true;
                    break;
                }
            }
